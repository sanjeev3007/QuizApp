...
// create quiz
export async function createEnglishQuiz(
  userid: string,
  grade: number,
  subjectId: number
) {
...
export async function updateEnglishQuiz({
  userid,
  questions,
  topic_id,
  quiz_id,
...
const fetchQuestionsByLevel = async (
  ...
  const { data, error } = await supabase.rpc("db_english_rpc", {
    level: level,
    rows_limit: limit,
    subject_topic: topic,
    uuids: questionIds,
...
export const getNumberOfCompletedEnglishQuiz = async (
  userid: string,
  subject_id: number
) => {
  const supabase = createClientComponentClient();
...
export async function getInCompletedEnglishQuiz(
  userId: string,
  subject_id: number
) {
  const supabase = createClientComponentClient();
